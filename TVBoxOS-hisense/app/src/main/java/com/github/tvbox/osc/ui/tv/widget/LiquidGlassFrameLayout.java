...
public class LiquidGlassFrameLayout extends FrameLayout {
    ...
    private int focusedContentColor;
    private int unfocusedContentColor;
    private long glassPulseStartMs = -1L;

    public LiquidGlassFrameLayout(Context context) {
        this(context, null);
    }

    public LiquidGlassFrameLayout(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public LiquidGlassFrameLayout(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context.getResources());
    }

    private void init(Resources resources) {
        cornerRadius = resources.getDimension(R.dimen.vs_26);
        ...
        focusedContentColor = ContextCompat.getColor(getContext(), R.color.apple_tv_selected_text_dark);
        unfocusedContentColor = ContextCompat.getColor(getContext(), R.color.apple_tv_text_primary);
    ...
    protected void drawGlass(Canvas canvas) {
        ...
        if (!LiquidGlassSnapshotManager.drawBackdrop(canvas, this, outlineRect, backdropPaint)) {
            glassHelper.draw(
                    canvas,
                    getWidth(),
                    getHeight(),
    ...
    protected void onAttachedToWindow() {
        ...
        applyFocusContrastIfNeeded();
    }

    @Override
    protected void onFocusChanged(boolean gainFocus, int direction, @Nullable Rect previouslyFocusedRect) {
        super.onFocusChanged(gainFocus, direction, previouslyFocusedRect);
        if (gainFocus && shouldUseFocusedGlass()) {
    ...
    private void applyFocusContrastIfNeeded() {
        Object tag = getTag();
        if (!(tag instanceof String) || !((String) tag).contains("contrast-focus")) {
            return;
        }
        applyFocusContrastRecursively(this, shouldUseFocusedGlass());
    }

    private void applyFocusContrastRecursively(View view, boolean focused) {
        if (view instanceof TextView) {
            ((TextView) view).setTextColor(focused ? focusedContentColor : unfocusedContentColor);
        } else if (view instanceof ImageView) {
            ((ImageView) view).setColorFilter(focused ? focusedContentColor : unfocusedContentColor, PorterDuff.Mode.SRC_IN);
        }
        if (view instanceof FrameLayout) {
            FrameLayout group = (FrameLayout) view;
            for (int i = 0; i < group.getChildCount(); i++) {
                applyFocusContrastRecursively(group.getChildAt(i), focused);
            }
